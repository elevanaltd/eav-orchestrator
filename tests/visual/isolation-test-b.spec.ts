/**
 * ISOLATION TEST B: Test editor WITH Y.js Collaboration but without content prop
 * This test verifies if the TipTap editor accepts input when Collaboration is enabled
 * but we don't pass the content prop (let Y.Doc manage it)
 */

import { test, expect } from '@playwright/test';

test.describe('Isolation Test B - Editor with fixed Collaboration', () => {
  test('should allow typing when Collaboration enabled without content prop', async ({ page }) => {
    // Navigate to the app
    ...
    // Type test text
    const testText = 'ISOLATION TEST B: This text was typed WITH Y.js Collaboration (fixed)!';
    ...
    if (content?.includes(testText)) {
      console.log('🎉 SUCCESS: Editor accepts input WITH Collaboration!');
      console.log('   Fix: Not passing content prop when using Y.js');
      console.log('   Y.js manages the document state properly now');
    } else {
      console.log('❌ FAIL: Editor still not accepting input with Collaboration');
      console.log('   Need to investigate Y.js initialization further');
    }
